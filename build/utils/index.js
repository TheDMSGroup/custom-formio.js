...
const FormioUtils = {
    ...
    eachComponent(components, fn, includeAll, path, parent) {
        ...
        components.forEach(component => {
            ...
            if (!noRecurse) {
                if (hasColumns) {
                    component.columns.forEach(column => FormioUtils.eachComponent(column.components, fn, includeAll, subPath(), parent ? component : null));
                } else if (hasRows) {
                    component.rows.forEach(row => row.forEach(column => FormioUtils.eachComponent(column.components, fn, includeAll, subPath(), parent ? component : null)));
                } else if (hasComps) {
     ...
     */
    parseFloat(value) {
        return parseFloat(_.isString(value) ? value.replace(/[^\de.+-]/gi, '') : value);
    },
    ...
    formatAsCurrency(value) {
        ...
        const parts = _.round(parsedValue, 2).toString().split('.');
        parts[0] = _.chunk(Array.from(parts[0]).reverse(), 3).reverse().map(part => part.reverse().join('')).join(',');
    ...
    checkCalculated(component, submission, rowData) {
        ...
        if (component.calculateValue) {
            ...
            if (_.isString(component.calculateValue)) {
                try {
                    _.set(rowData, component.key, new Function('component', 'data', 'row', 'util', 'moment', `var value = [];${component.calculateValue.toString()}; return value;`)(component, data, row, this, moment));
                } catch (e) {
    ...
    checkCustomConditional(component, custom, row, data, variable, onError) {
        try {
            return new Function('component', 'data', 'row', 'util', 'moment', `var ${variable} = true; ${custom.toString()}; return ${variable};`)(component, data, row, this, moment);
        } catch (e) {
